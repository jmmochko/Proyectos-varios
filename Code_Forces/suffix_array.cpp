...
int main(){
    ...
    fore(i,1,sa.size()){
        int j = 0;
        auto it = lower_bound(all(sa), make_pair(sa[i],0));
        while(sa[i] == qs[(it - sa.begin()) + j].first){
            fnal.push_back(qs[(it - sa.begin()) + j]);
        }
    }
    fore(i,0,fnal.size()){
        cout<<fnal[i].first<<" "<<fnal[i].second<<'\n';
    }
    return 0;
}