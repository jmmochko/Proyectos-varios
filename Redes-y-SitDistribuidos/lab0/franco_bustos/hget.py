...
# Este lab0 fue el que entregué el año pasado, como es igual al anterior
# copié y pegué el mismo hecho por mí.
...
def connect_to_server(server_name):
    ...
    # Se utiliza la funcion de socket gethostbyname que transforma una url
    # valida a su correspondiente ip
    ...
    # Se utiliza la funcion socket de socket, que toma la familia de
    # direccion, en este caso como usamos IPv4, se usa la constante de socket
    # AF_INET. Tambien toma un tipo, en nuestro caso que usamos TCP se llama
    # a la constante de socket SOCK_STREAM 
    s.connect((ip_address,HTTP_PORT))
    # Se utiliza la funcion de socket connect, que toma una direccion ip (o
    # una url) y un puerto al que se quiere comunicar 
