...
def connect_to_server(server_name):
    ...
    # Comentario alumno:
    # Hay que hacerle un poco de procesamiento al server name para pasarlo a IDNA

    server_name = str(server_name).encode('idna')

    print("中文.tw -> Encoded to " + repr(server_name))
    
    # Comentario alumno:
    # >>> print("Encoded to " + str(server_name))
    # 中文.tw -> Encoded to b'xn--fiq228c.tw'

    # comentario alumno:
    # "gethostbyname" busca la direccion ip del server_name dado
    ...
    # Comentario alumno:
    # Creo el socket con addr family AF_INET y del tipo SOCK_STREAM 
    # ya que es para IPv4 y el protocolo TCP
